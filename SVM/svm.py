...
def testRbf(k1=1.3):
    ...
    dataArr0 = datMat[labelMat.A[:,0]>0,:]
    dataArr1 = datMat[labelMat.A[:,0]<0,:]
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(dataArr0[:,0],dataArr0[:,1], marker='s', s=90)
    ax.scatter(dataArr1[:,0],dataArr1[:,0], marker='o', s=50, c='red')
    plt.title('Support Vectors Circled')
    ...
    for i in xrange(sVs.shape[0]):
        circle = Circle(sVs[i,:].A[0], 0.05, facecolor='none', edgecolor=(0,0.8,0.8), linewidth=1, alpha=0.5)
        ax.add_patch(circle)
